...
$statement = $pdo->prepare('SELECT * FROM '. $_SESSION['user_name'].' WHERE id = :id');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($error)) {
        $statement = $pdo->prepare('UPDATE ' .$_SESSION['user_name']. ' SET name = :name , phone_no = :phone_no, email = :email, website = :website, facebook = :facebook WHERE id = :id');
...
<ul>
    ...
    <form  action="" method="post">
        <h1>UPDATE CONTACT <br> <b><?php echo $contact['name'] ?> </h1>
        <?php 
        if (!empty($error)) {
